...
class CocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        file = open(annotation_file_path)

        anns = json.load(file)

        self.image_id_annotations = defaultdict(dict)
        ...
        for annotation in anns['annotations']:
            ...
            if not 'labels' in self.image_id_annotations[image_id]:
                self.image_id_annotations[image_id]['labels'] = []
                self.image_id_annotations[image_id]['bbox'] = []
            self.image_id_annotations[image_id]['labels'].append(annotation['category_id'])
            self.image_id_annotations[image_id]['bbox'].append(annotation['bbox'])

    def __len__(self):
        return len(self.image_id_annotations)

    def __getitem__(self, index):
        image_id = sorted(self.image_id_annotations.keys())[index]
        image_path = sorted(os.listdir(self.dataset_dir))[index]
        image = Image.open(os.path.join(self.dataset_dir, image_path)).convert('RGB')

        gt_boxes = torch.tensor(self.image_id_annotations[image_id]['bbox'], dtype=torch.float32)
        ...
        gt_labels = torch.tensor(self.image_id_annotations[image_id]['labels']).unsqueeze(-1)
